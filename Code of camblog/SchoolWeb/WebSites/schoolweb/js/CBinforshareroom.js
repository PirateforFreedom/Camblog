...
//返回：用户信息和发布的信息
var detail_div = 1;
function GetUserinforbyname_atonload(usernameonline) {
    var strpictrue;
    var strpostsum;
    var strstatement;
    var strdate_time;
    var strcountry;
    var struser_name_tiaoshu;
    var strfollowingsum;
    var strfollowersum;
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each($.find("Table1", data), function () {
               ...
               strstatement = $(this).find("statement").text();
                ...
                struser_name_tiaoshu = $(this).find("user_name_tiaoshu").text();
...
function GetDataSetforpersonpostbynameonload_for_comment(usernameonline) {

    var totaltweet = -1;
    var detailfrist_div = 0;
    $.ajax({
        url: "WebService.asmx/GetDataSetforpersonpostforuser__for_comment",
        type: "Post",
        dataType: "xml",
        data: { username1: usernameonline },
        success: function (data) {
            var strpostcontent = "";
            var strpostdate = "2019"
            var usernamer1 = ""
            var picturer = ""
            $.each($.find("Table1", data), function () {

                strpostcontent = $(this).find("user_reply_content").text();
                usernamer1 = $(this).find("user_replyer").text();
               
                strpostdate = $(this).find("user_comment_date").text();
                picturer = $(this).find("user_follower_pictrue").text();
                strpostcontent = strpostcontent.replace(/brHH/g, "<br>");
                strpostcontent = strpostcontent.replace(/\s/g, "&nbsp;");
                totaltweet++;
                detailfrist_div++;
                add_divonload_for_commment(totaltweet, detailfrist_div, strpostcontent, strpostdate, picturer, usernamer1)
            });
...
function onload_information() {
   ...
   var usernameonline = get_cookie("username");
    //取用户信息
    GetUserinforbyname_atonload(usernameonline);
    GetDataSetforpersonpostbynameonload(usernameonline);
}
function onload_information_for_comment(user_post_personsd) {
   
   // var usernameonline = 'tom_1';
  
    //var usernameonline = get_cookie("username");
    //取用户信息GetDataSetforpersonpostforuser__for_comment
   // GetUserinforbyname_atonload_for_comment(usernameonline);
    GetDataSetforpersonpostbynameonload_for_comment(user_post_personsd);
}
//跳转页面，传递参数
...
function addcommentdiv(strtxt, user_post_personsd, usernamerwww) {
   // var strtxt = document.getElementById(textmabsolute.id).value;
   ...
   // alert(strtxt)
    ...
    $.ajax({
        url: "WebService.asmx/Getfollowingforuser_for_comment",
        type: "Post",
        dataType: "xml",
        async: false,
        data: { folloingusername: user_post_personsd },
        success: function (data) {
            $.each($.find("Table1", data), function () {
               // var strpictrue = $(this).find("user_pictrue").text();
               // pictrue_path = strpictrue;
                var strpostsum = $(this).find("user_post_coment_sum").text();
                poss = strpostsum;
                var strpostsum1 = $(this).find("user_post_comment_tiaoshu").text();
                 poss1 = strpostsum1;
  ...
  $.ajax({
        ...
        url: "WebService.asmx/InsertCommentTxt", //调用WebService的地址和方法名称组合 ---- WsURL/方法名
        data: "{strtxt:'" + strtxt + "',usernamef:'" + username + "',userposttime:'" + user_post_personsd + "',pic:'"+pictrue_path+"'}",         //这里是要传递的参数，格式为 data: "{paraName:paraValue}",下面将会看到       
        dataType: 'json',
        success: function (result) {     //回调函数，result，返回值
            ...
            if (json == 'no') {
                ...
                var strpostcontent = strtxt;
                strpostcontent = strpostcontent.replace(/brHH/g, "<br>");
                strpostcontent = strpostcontent.replace(/\s/g, "&nbsp;");
                add_commentdiv(strpostcontent, mytime, pictrue_path, poss, poss1, username);
                document.getElementById(textmabsolute.id).value = "";
            }
        }
    });
}
function insertlikes(user_post_personsd, likenumdd) {
    $.ajax({
        type: "POST",   //访问WebService使用Post方式请求
        contentType: "application/json", //WebService 会返回Json类型
        url: "WebService.asmx/InsertCommentlikes", //调用WebService的地址和方法名称组合 ---- WsURL/方法名
        data: "{userposttime:'" + user_post_personsd + "'}",         //这里是要传递的参数，格式为 data: "{paraName:paraValue}",下面将会看到       
        dataType: 'json',
        success: function (result) {     //回调函数，result，返回值
...
//var totaltweet = 0;
//发布时动态创建div
function add_commentdiv(txtet, str2, str3, posss, poss1, username) {
    ...
    person_contentflydiv_buttoncomment.onclick = function person_contentflydiv_button_onclik() {
        //  Clinkcommentbutton_event(detail_div, txtet, str2, str3, usernamer);
    ...
    person_contentflydiv_botombar.appendChild(person_contentflydiv_buttoncomment);
        ...
        document.getElementById("detailswerolp").appendChild(person_contentflydiv);
   ...
   $.ajax({
       ...
       success: function (result) {     //回调函数，result，返回值
           ...
           if (json == 'no') {
               ...
               strpostcontent = strpostcontent.replace(/brHH/g, "<br>");
               strpostcontent = strpostcontent.replace(/\s/g, "&nbsp;");
               add_div(strpostcontent, mytime, pictrue_path, poss, poss1, username);
...
function add_divonload_for_commment(totaltweet, detailfrist_div, txtet, str2, str3, usernamer) {
    //创建最动态内容div
    ...
    person_contentflydiv_buttoncomment.onclick = function person_contentflydiv_button_onclik() {
       // Clinkcommentbutton_event(txtet, str2, str3, usernamer);
...
function add_div(txtet, str2, str3, posss, poss1, username) {
    ...
    person_contentflydiv_buttoncomment.onclick = function person_contentflydiv_button_onclik() {
        ...
        Clinkcommentbutton_event(txtet, str2, str3, username);
...
/* var person_contentflydiv = document.createElement('div');
 ...
 person_contentflydiv.style.borderTopColor = "#E0E0E0";
 person_contentflydiv.style.cssFloat = "left"
 person_contentflydiv.style.display = "inline-block";
 detail_div++;
 var person_contentflydiv_picbar = document.createElement('div');
 person_contentflydiv_picbar.id = "details" + detail_div;
 var stylepicbar = document.createAttribute('style');
 ...
 person_contentflydiv_txt.style.fontSize = "15px";
 person_contentflydiv_txt.style.color = "#101010";
 person_contentflydiv_txt.style.display = "inline-block";
 person_contentflydiv_txtbar.appendChild(person_contentflydiv_txt);
 detail_div++;
 var person_contentflydiv_botombar = document.createElement('div');
 person_contentflydiv_botombar.id = "details" + detail_div;
