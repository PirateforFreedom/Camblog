function onload_information() {
    ...
    $.ajax({
        url: "WebService.asmx/SearchforInformation",      
        type: "POST",
        dataType: "xml",        
        data:{ name: username },
        success: function (data) {
            $.each($.find("table",data),function(){
            var user_pictrue = $(this).find("user_pictrue").text();
            var statement = $(this).find("statement").text();
            var major_in = $(this).find("major_in").text();
            var country = $(this).find("country").text();
            var school_name = $(this).find("school_name").text();
            var email = $(this).find("email").text();
            var birth = $(this).find("birth").text();
            var enroll = $(this).find("enroll").text();
            $("#nickname").attr("value", username);
            $("#country").attr("value", country);
            $("#statement").text(statement);
            $("#specialty").attr("value", major_in);
            $("#school").attr("value", school_name);
            $("#email").attr("value", email);
            $("#birth").text(birth);           
            $("#enrolltime").text(enroll);           
            $("#cvs").attr("src", user_pictrue);
            });
...
function AlterInformation() {
    ...
    $.ajax({
        type: "POST",   //访问WebService使用Post方式请求
        contentType: "application/json", //WebService 会返回Json类型
        url: "WebService.asmx/AlterInformation", //调用WebService的地址和方法名称组合 ---- WsURL/方法名
        data: "{nickname:'" + name + "',specialty:'" + specialty + "',school:'" +
            school + "',email:'" + email +  "',country:'" + country + "',oldname:'"+username+"'}",         //这里是要传递的参数，格式为 data: "{paraName:paraValue}",下面将会看到       
        dataType: 'json',
        success: function (result) {     //回调函数，result，返回值
            var json = result.d
            if (json == "yes") {
                alert("用户信息修改成功");
                window.location.href = "CBPersonInformation.aspx?username=" + name;
            }
            else {
                alert("信息修改失败");
            }
...
function backPersonroom() {
    var name = document.getElementById("nickname").value;
    window.location.href = "CBPersonroom.aspx?username=" + name;
}
...
function checkAll() {
    ...
    if (name && specialty && school && email && birth && enroll && country) {  
        if (nickname == "yes") {         
                AlterInformation();
                alert("信息保存完毕")
                return true;         
