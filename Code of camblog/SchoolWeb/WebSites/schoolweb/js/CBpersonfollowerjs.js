...
function onload_information() {
   ...
   // var usernameonline = 'tom';
    ...
    $.ajax({
        url: "WebService.asmx/Getfollowingforuser",
        type: "Post",
        dataType: "xml",
        ...
        success: function (data) {
            ...
            var strpostcon= ""
            $.each($.find("Table1", data), function () {
                strpostcontent = $(this).find("user_follower_statement").text();
               strpostdate = $(this).find("user_follower_date").text();
                strpostname = $(this).find("user_follower_name").text();
                strpostpic = $(this).find("user_follower_pictrue").text();
               strpostcon = $(this).find("user_follower_conntry").text();
